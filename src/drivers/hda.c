...
#define HDA_PIN_WIDGET_CTRL_OUT_EN (1u << 6)
...
// Program the converter/stream binding and enable output on the selected pin.
bool hda_codec0_configure_output_path(uint8_t dac_nid,
    ...
    if (!g_hda.codec_present) return false;
    if (!g_hda.codec_mask) return false;
    if (!dac_nid || !pin_nid) return false;
    if (stream_tag == 0 || stream_tag > 15) return false; // valid stream IDs are 1..15

    uint8_t codec = g_hda.primary_codec;

    // Program the converter format (sample rate/width/channels)
    if (!hda_send_verb_best_available(codec, dac_nid,
                                      HDA_VERB_SET_CONVERTER_FORMAT,
                                      format, NULL)) {
        return false;
    }

    // Bind the converter to the chosen stream/tag with channel 0
    uint16_t stream_payload = (uint16_t)((stream_tag << 4) | 0x0);
    if (!hda_send_verb_best_available(codec, dac_nid,
